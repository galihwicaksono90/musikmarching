...
select p.id, p.invoice_serial, p.account_id, p.score_id, p.price, p.title, p.payment_proof_url, p.paid_at, p.is_verified, p.verified_at, p.created_at, p.updated_at, p.deleted_at, a.name from purchase p
...
const verifyPurchase = `-- name: VerifyPurchase :one
update purchase set
is_verified = true,
verified_at = now()
where id = $1
returning id, invoice_serial, account_id, score_id, price, title, payment_proof_url, paid_at, is_verified, verified_at, created_at, updated_at, deleted_at
`

func (q *Queries) VerifyPurchase(ctx context.Context, id uuid.UUID) (Purchase, error) {
	row := q.db.QueryRow(ctx, verifyPurchase, id)
	var i Purchase
	err := row.Scan(
		&i.ID,
		&i.InvoiceSerial,
